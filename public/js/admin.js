  ...
  socket.emit("admin_list_messages_by_user", params, messages => {
    ...
    messages.forEach(message => {
      ...
      if (message.admin_id === null) {
        ...
        createDiv.innerHTML += `<span class="admin_date">${dayjs(message.createdAt).format("DD/MM/YYY HH:mm:ss")}`;
      }
...
socket.on("admin_receive_message", params => {
  const connection =
    connectionsUsers.find((connection) => connection.socket_id = params.socket_id);
  ...
  createDiv.innerHTML += `<span class="admin_date">${dayjs(params.message.createdAt).format("DD/MM/YYYY HH:mm:ss")}</span>`;
